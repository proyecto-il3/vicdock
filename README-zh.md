...
Vicdock 能够帮你在 **Docker** 上快速搭建 **Laravel** 应用。

就像 Laravel Homestead 一样，但是 Docker 替换了 Vagrant。

> 先在使用 Vicdock，然后再学习它们。
...
- [Intro](#Intro)
	...
	- [Vicdock VS Homestead](#vicdock-vs-homestead)
- [Demo Video](#Demo)
...
- [Documentation](#Documentation)
	...
	- [Misc](#Misc)
		...
		- [Debugging](#debugging)
		- [Upgrading Vicdock](#upgrading-vicdock)
- [Help & Questions](#Help)
...
<a name="vicdock-vs-homestead"></a>
### Vicdock Homestead 对比

Vicdock and [Homestead](https://laravel.com/docs/master/homestead) 给你一个完整的虚拟开发环境。(不需要安装和配置软件在你自己的每一个操作系统)。

Homestead 是一个工具,为你控制虚拟机(使用 Homestead 特殊命令)。Vagrant 可以管理你的管理虚容器。

运行一个虚拟容器比运行一整个虚拟机快多了 **Vicdock 比 Homestead 快多了**

<a name="Demo"></a>
## 演示视频
还有什么比**演示视频**好：

- Vicdock [v4.0](https://www.youtube.com/watch?v=TQii1jDa96Y)
- Vicdock [v2.2](https://www.youtube.com/watch?v=-DamFMczwDA)
- Vicdock [v0.3](https://www.youtube.com/watch?v=jGkyO6Is_aI)
- Vicdock [v0.1](https://www.youtube.com/watch?v=3YQsHe6oF80)
...
```bash
git submodule add https://github.com/vicdock/vicdock.git
```
...
- 更新到 [Docker Desktop](https://www.docker.com/products/docker-desktop/) Mac/Windows 版本 (建议). 查看 [Upgrading Vicdock](#upgrading-vicdock)
- 使用 Vicdock v3.* (访问 `Vicdock-ToolBox` [分支](https://github.com/vicdock/vicdock/tree/Vicdock-ToolBox)).
如果您使用的是 **Docker Desktop**(Mac / Windows 版本)甚至是 Linux 版本,通常可以继续阅读这个文档，Vicdock v4 以上版本将仅支持 **Docker Desktop**。

1 - 运行容器: *(在运行 `docker-compose` 命令之前，确认你在 `vicdock` 目录中*
...
增加 `--user=vicdock` (例如 `docker-compose exec --user=vicdock workspace bash`) 作为您的主机的用户创建的文件. (你可以从 `docker-compose.yml`修改 PUID (User id) 和 PGID (group id) 值 ).
...
系统默认 Vicdock 假定 Laravel 应用在 vicdock 的父级目录中
...
5 - 回到 Vicdock 安装步骤,看看如何编辑 `.env` 的文件。
...
增加 `--user=vicdock` (例如 `docker-compose exec --user=vicdock workspace bash`) 作为您的主机的用户创建的文件.
...
1 - 克隆 `https://github.com/vicdock/php-fpm`.
...
<a name="upgrading-vicdock"></a>
### Vicdock 升级


从 Docker Toolbox (VirtualBox) 移动到 Docker Native (for Mac/Windows)，需要从 Vicdock v3.* 升级到 v4.*:
...
3. 升级 Vicdock 到 `v4.*.*` (`git pull origin master`)
4. 像之前一样使用 Vicdock: `docker-compose up -d nginx mysql`.
...
#### 阅读我们的 [贡献说明](https://github.com/vicdock/vicdock/blob/master/CONTRIBUTING.md)

<a name="Help"></a>
## 帮助 & 问题

从聊天室 [Gitter](https://gitter.im/Vicdock/vicdock) 社区获取帮助和支持.

你也可以打开 Github 上的 [issue](https://github.com/vicdock/vicdock/issues) (将被贴上问题和答案) 或与大家讨论 [Gitter](https://gitter.im/Vicdock/vicdock).
...
- [Contributors](https://github.com/vicdock/vicdock/graphs/contributors)
- [Supporters](https://github.com/vicdock/vicdock/issues?utf8=%E2%9C%93&q=)


## 许可证

[MIT License](https://github.com/vicdock/vicdock/blob/master/LICENSE) (MIT)
